 ...
 */
export function transformResultItem(resultItem, formatImageUrl) {
    const { value, matched_terms: matchedTerms, data, ...otherFields } = resultItem;
    if (!data || typeof data !== 'object' || !data.id || !value) {
        return null;
    }
    const { id, variation_id: variationId, description, url, image_url: imageUrl, price, sale_price: salePrice, rating, reviews_count: reviewsCount, tags, sl_campaign_id: slCampaignId, sl_campaign_owner: slCampaignOwner, badge, ...otherMetadataFields } = data;
